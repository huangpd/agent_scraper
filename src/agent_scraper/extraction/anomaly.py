...
def _explain(feats: np.ndarray, idx: int, parsed: list[dict]) -> str:
    ...
    强信号（域名异常、可疑关键词）单独即可成立；
    弱信号（扩展名/前缀占比低、结构偏差）需要 ≥2 个叠加才报告，
    避免"仓库里有几个 .py 文件"之类的正常变异被误报。
    """
    f = feats[idx]
    strong: list[str] = []  # 高置信度信号
    weak: list[str] = []    # 低置信度信号，需叠加

    # ── 强信号 ──
    if f[0] < 0.05:
        strong.append(f"域名 '{parsed[idx]['domain']}' 在批次中极少出现")
    elif f[0] < 0.1:
        weak.append(f"域名 '{parsed[idx]['domain']}' 在批次中较少出现")
    ...
    if f[3] > 0:
        ...
        strong.append(f"路径含可疑关键词 '{kw}'")

    if f[4] > 3:
        strong.append(f"查询参数数量异常 ({int(f[4])} 个)")

    # ── 弱信号（需叠加） ──
    if f[1] < 0.05:
        weak.append(f"扩展名 '{parsed[idx]['ext'] or '(无)'}' 在批次中极少出现")

    if f[2] < 0.05:
        weak.append(f"路径前缀 '/{parsed[idx]['prefix']}/' 在批次中极少出现")

    if f[5] > 1.0:
        weak.append(f"长度偏离中位数 {f[5]:.0%}")

    if f[6] > 1.0:
        weak.append(f"路径深度偏离中位数 {f[6]:.0%}")

    # 有强信号 → 全部输出；仅弱信号 → 需 ≥2 个才报告
    if strong:
